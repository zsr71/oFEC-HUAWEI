...
- window 统计不是天然独立任务，而是一次完整运行后的切片结果
- 如果按 window 拆任务，会破坏当前 pipeline 的自然执行路径
- 对实现复杂度帮助不大，反而会让统计链更难维护

因此推荐的任务组织方式是：

- 一个任务对应一组固定配置下的一个 `(Eb/N0, seed)`
- 每个任务独立跑一条完整 pipeline
- 任务结束后返回该次运行的 per-window 原始统计
- 主线程再按：
  - `Eb/N0`
  - `window_idx`
...
- 方案 C 本来就是多 seed 聚合
- 每个 seed 完全独立
- 聚合口径天然就是对同一 `window_idx` 的 `errs/bits` 累加

因此，第一版如果要加并行，最应该先做的就是 seed 并行。

#### 2. 其次支持 Eb/N0 并行

如果一次要测多个工作点，也可以按 `Eb/N0` 并行。

但建议实现上保持简单：

- 外层可以是多个 `Eb/N0`
- 内层每个 `Eb/N0` 下仍然按 seed 聚合
...
文档明确不建议引入 window 级并行。第一版和后续版本都不需要把 window 作为独立任务单元。
