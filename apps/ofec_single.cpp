...
static const std::vector<int> kSisoActiveList = {32, 32, 32, 32,16,8}; // 每个 tile 允许参与 SISO 的行数预算
static const std::vector<int> kHiHoActiveList = {32, 32, 32, 32, 16, 16}; // 每个 tile 允许参与 HIHO 硬解码的行数预算
...
static const std::vector<float> kHybridHardLlrMagList = {0,0,0,99,99,99};      // 按 tile 覆盖 hybrid hard-finish |LLR| 幅度；空=沿用 kHybridHardLlrMag
