...
extern template DecoderCoreResult<int8_t> Decoder_Core_ebchPF<int8_t>(const matrix::Matrix<int8_t>&,
                                                                      ...
                                                                      const std::vector<bool>*,
                                                                      const std::vector<uint8_t>*);
extern template DecoderCoreResult<float> Decoder_Core_topk_pruned<float>(const matrix::Matrix<float>&,
                                                                         const matrix::Matrix<float>&,
                                                                         bool,
                                                                         const newcode::Params&,
                                                                         const std::vector<bool>*,
                                                                         const std::vector<uint8_t>*);
extern template DecoderCoreResult<int8_t> Decoder_Core_topk_pruned<int8_t>(const matrix::Matrix<int8_t>&,
                                                                           const matrix::Matrix<int8_t>&,
                                                                           bool,
                                                                           const newcode::Params&,
                                                                           const std::vector<bool>*,
                                                                           const std::vector<uint8_t>*);
extern template DecoderCoreResult<float> Decoder_Core_global_pair<float>(const matrix::Matrix<float>&,
                                                                         const matrix::Matrix<float>&,
                                                                         bool,
                                                                         const newcode::Params&,
                                                                         const std::vector<bool>*,
                                                                         const std::vector<uint8_t>*);
extern template DecoderCoreResult<int8_t> Decoder_Core_global_pair<int8_t>(const matrix::Matrix<int8_t>&,
                                                                           const matrix::Matrix<int8_t>&,
                                                                           bool,
                                                                           const newcode::Params&,
                                                                           const std::vector<bool>*,
                                                                           const std::vector<uint8_t>*);
extern template DecoderCoreResult<float> Decoder_Core_group_minima<float>(const matrix::Matrix<float>&,
                                                                          const matrix::Matrix<float>&,
                                                                          bool,
                                                                          const newcode::Params&,
                                                                          const std::vector<bool>*,
                                                                          const std::vector<uint8_t>*);
extern template DecoderCoreResult<int8_t> Decoder_Core_group_minima<int8_t>(const matrix::Matrix<int8_t>&,
                                                                            const matrix::Matrix<int8_t>&,
                                                                            bool,
                                                                            const newcode::Params&,
                                                                            const std::vector<bool>*,
...
extern template DecoderCoreResult<qfloat::qfloat<N>> Decoder_Core_ebchPF<qfloat::qfloat<N>>( \
    const matrix::Matrix<qfloat::qfloat<N>>& lin_matrix, const matrix::Matrix<qfloat::qfloat<N>>& lch_matrix, bool, const newcode::Params&, \
    const std::vector<bool>*, const std::vector<uint8_t>*); \
extern template DecoderCoreResult<qfloat::qfloat<N>> Decoder_Core_topk_pruned<qfloat::qfloat<N>>( \
    const matrix::Matrix<qfloat::qfloat<N>>& lin_matrix, const matrix::Matrix<qfloat::qfloat<N>>& lch_matrix, bool, const newcode::Params&, \
    const std::vector<bool>*, const std::vector<uint8_t>*); \
extern template DecoderCoreResult<qfloat::qfloat<N>> Decoder_Core_global_pair<qfloat::qfloat<N>>( \
    const matrix::Matrix<qfloat::qfloat<N>>& lin_matrix, const matrix::Matrix<qfloat::qfloat<N>>& lch_matrix, bool, const newcode::Params&, \
    const std::vector<bool>*, const std::vector<uint8_t>*); \
extern template DecoderCoreResult<qfloat::qfloat<N>> Decoder_Core_group_minima<qfloat::qfloat<N>>( \
    const matrix::Matrix<qfloat::qfloat<N>>& lin_matrix, const matrix::Matrix<qfloat::qfloat<N>>& lch_matrix, bool, const newcode::Params&, \
    const std::vector<bool>*, const std::vector<uint8_t>*);
