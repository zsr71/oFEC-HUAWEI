...
// ★ 新增参数 tile_top_row_global：该 tile 在全局矩阵中的顶行号（用于按 oFEC 公式索引“旧信息”）
template <typename LLR>
Matrix<LLR> process_tile(const Matrix<LLR>& tile_in, const Params& p, std::size_t tile_top_row_global);
...
extern template void process_window<int8_t>(Matrix<int8_t>&, std::size_t, std::size_t, const Params&,
                                            std::size_t, std::size_t, std::size_t);
extern template Matrix<float>  process_tile<float >(const Matrix<float>&,  const Params&, std::size_t);
extern template Matrix<int8_t> process_tile<int8_t>(const Matrix<int8_t>&, const Params&, std::size_t);
...
extern template void process_window<newcode::qfloat<5>>(Matrix<newcode::qfloat<5>>&, std::size_t, std::size_t, const Params&,
                                                        std::size_t, std::size_t, std::size_t);
extern template Matrix<newcode::qfloat<4>> process_tile<newcode::qfloat<4>>(const Matrix<newcode::qfloat<4>>&, const Params&, std::size_t);
extern template Matrix<newcode::qfloat<5>> process_tile<newcode::qfloat<5>>(const Matrix<newcode::qfloat<5>>&, const Params&, std::size_t);
