...
template<typename LLR>
DecoderCoreResult<LLR> Decoder_Core_topk_pruned(const matrix::Matrix<LLR>& lin_matrix,
                                                const matrix::Matrix<LLR>& lch_matrix,
                                                bool use_hard_decode,
                                                const newcode::Params& p,
                                                const std::vector<bool>* early_stop_row_flags,
...
{
  return Decoder_Core_impl(lin_matrix, lch_matrix, use_hard_decode, p,
                           &chase_decode_256_topk_pruned<LLR>,
                           early_stop_row_flags,
                           mux_state);
}

template<typename LLR>
DecoderCoreResult<LLR> Decoder_Core_global_pair(const matrix::Matrix<LLR>& lin_matrix,
                                                const matrix::Matrix<LLR>& lch_matrix,
                                                bool use_hard_decode,
                                                const newcode::Params& p,
                                                const std::vector<bool>* early_stop_row_flags,
...
{
  return Decoder_Core_impl(lin_matrix, lch_matrix, use_hard_decode, p,
                           &chase_decode_256_global_pair<LLR>,
                           early_stop_row_flags,
                           mux_state);
}

template<typename LLR>
DecoderCoreResult<LLR> Decoder_Core_group_minima(const matrix::Matrix<LLR>& lin_matrix,
                                                 const matrix::Matrix<LLR>& lch_matrix,
                                                 bool use_hard_decode,
                                                 const newcode::Params& p,
                                                 const std::vector<bool>* early_stop_row_flags,
...
{
  return Decoder_Core_impl(lin_matrix, lch_matrix, use_hard_decode, p,
                           &chase_decode_256_group_minima<LLR>,
                           early_stop_row_flags,
                           mux_state);
}
...
template DecoderCoreResult<float> Decoder_Core_ebchPF<float>(const matrix::Matrix<float>&,
                                                             ...
                                                             const std::vector<bool>*,
                                                             const std::vector<uint8_t>*);
template DecoderCoreResult<float> Decoder_Core_topk_pruned<float>(const matrix::Matrix<float>&,
                                                                  const matrix::Matrix<float>&,
                                                                  bool,
                                                                  const newcode::Params&,
                                                                  const std::vector<bool>*,
                                                                  const std::vector<uint8_t>*);
template DecoderCoreResult<float> Decoder_Core_global_pair<float>(const matrix::Matrix<float>&,
                                                                  const matrix::Matrix<float>&,
                                                                  bool,
                                                                  const newcode::Params&,
                                                                  const std::vector<bool>*,
                                                                  const std::vector<uint8_t>*);
template DecoderCoreResult<float> Decoder_Core_group_minima<float>(const matrix::Matrix<float>&,
                                                                   const matrix::Matrix<float>&,
                                                                   bool,
                                                                   const newcode::Params&,
                                                                   const std::vector<bool>*,
...
template DecoderCoreResult<int8_t> Decoder_Core_ebchPF<int8_t>(const matrix::Matrix<int8_t>&,
                                                               ...
                                                               const std::vector<bool>*,
                                                               const std::vector<uint8_t>*);
template DecoderCoreResult<int8_t> Decoder_Core_topk_pruned<int8_t>(const matrix::Matrix<int8_t>&,
                                                                    const matrix::Matrix<int8_t>&,
                                                                    bool,
                                                                    const newcode::Params&,
                                                                    const std::vector<bool>*,
                                                                    const std::vector<uint8_t>*);
template DecoderCoreResult<int8_t> Decoder_Core_global_pair<int8_t>(const matrix::Matrix<int8_t>&,
                                                                    const matrix::Matrix<int8_t>&,
                                                                    bool,
                                                                    const newcode::Params&,
                                                                    const std::vector<bool>*,
                                                                    const std::vector<uint8_t>*);
template DecoderCoreResult<int8_t> Decoder_Core_group_minima<int8_t>(const matrix::Matrix<int8_t>&,
                                                                     const matrix::Matrix<int8_t>&,
                                                                     bool,
                                                                     const newcode::Params&,
                                                                     const std::vector<bool>*,
...
template DecoderCoreResult<qfloat::qfloat<N>> Decoder_Core_ebchPF<qfloat::qfloat<N>>( \
    const matrix::Matrix<qfloat::qfloat<N>>& lin_matrix, const matrix::Matrix<qfloat::qfloat<N>>& lch_matrix, bool, const newcode::Params&, \
    const std::vector<bool>*, const std::vector<uint8_t>*); \
template DecoderCoreResult<qfloat::qfloat<N>> Decoder_Core_topk_pruned<qfloat::qfloat<N>>( \
    const matrix::Matrix<qfloat::qfloat<N>>& lin_matrix, const matrix::Matrix<qfloat::qfloat<N>>& lch_matrix, bool, const newcode::Params&, \
    const std::vector<bool>*, const std::vector<uint8_t>*); \
template DecoderCoreResult<qfloat::qfloat<N>> Decoder_Core_global_pair<qfloat::qfloat<N>>( \
    const matrix::Matrix<qfloat::qfloat<N>>& lin_matrix, const matrix::Matrix<qfloat::qfloat<N>>& lch_matrix, bool, const newcode::Params&, \
    const std::vector<bool>*, const std::vector<uint8_t>*); \
template DecoderCoreResult<qfloat::qfloat<N>> Decoder_Core_group_minima<qfloat::qfloat<N>>( \
    const matrix::Matrix<qfloat::qfloat<N>>& lin_matrix, const matrix::Matrix<qfloat::qfloat<N>>& lch_matrix, bool, const newcode::Params&, \
    const std::vector<bool>*, const std::vector<uint8_t>*);
