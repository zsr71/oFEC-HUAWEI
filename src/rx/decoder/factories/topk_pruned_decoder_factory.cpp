...
template <int NBITS>
void decode_topk_pruned_qfloat(const DecodeRequest& request, DecodeResult& result) {
  ...
  if (!request.quiet) {
    std::cout << "[INFO] (" << request.label
              << ") Chase-topk-pruned decoder running in qfloat::qfloat<"
              << NBITS << ">\n";
  }
  ...
  auto decoded = ofec_decode_llr_topk_pruned(quantized, request.params,
                                             &result.tile_stats,
...
void decode_topk_pruned_quantized(const DecodeRequest& request,
                                  DecodeResult& result) {
  switch (request.quant_bits) {
    case 2:  decode_topk_pruned_qfloat<2 >(request, result); break;
    case 3:  decode_topk_pruned_qfloat<3 >(request, result); break;
    case 4:  decode_topk_pruned_qfloat<4 >(request, result); break;
    case 5:  decode_topk_pruned_qfloat<5 >(request, result); break;
    case 6:  decode_topk_pruned_qfloat<6 >(request, result); break;
    case 7:  decode_topk_pruned_qfloat<7 >(request, result); break;
    case 8:  decode_topk_pruned_qfloat<8 >(request, result); break;
    case 9:  decode_topk_pruned_qfloat<9 >(request, result); break;
    case 10: decode_topk_pruned_qfloat<10>(request, result); break;
    case 11: decode_topk_pruned_qfloat<11>(request, result); break;
    case 12: decode_topk_pruned_qfloat<12>(request, result); break;
    case 13: decode_topk_pruned_qfloat<13>(request, result); break;
    case 14: decode_topk_pruned_qfloat<14>(request, result); break;
    case 15: decode_topk_pruned_qfloat<15>(request, result); break;
    default:
      throw std::runtime_error(
          "[ERROR] Unsupported quant_bits for chase_topk_pruned decoder: " +
          std::to_string(request.quant_bits));
  }
}

class ChaseTopkPrunedDecoder final : public IDecoder {
 public:
  DecodeResult decode(const DecodeRequest& request) override {
    ...
    switch (request.format) {
      case LlrFormat::Float:
        if (!request.quiet) {
          std::cout << "[INFO] (" << request.label
                    << ") Chase-topk-pruned decoder running in FLOAT\n";
        }
        if (!request.quiet) {
        ...
        result.pre_decoder_llr = request.channel_llr;
        result.post_decoder_llr =
            ofec_decode_llr_topk_pruned(result.pre_decoder_llr, request.params,
                                        &result.tile_stats,
                                        request.normalize_extrinsic,
                                        request.tx_llr_ref);
        break;
      ...
      case LlrFormat::Quantized:
        decode_topk_pruned_quantized(request, result);
        break;
    }
...
static void ensure_registered() {
  ...
  std::call_once(once, [] {
    register_decoder_factory([](const std::string& name)
      ...
      if (name == "chase_topk_pruned") {
        return std::make_unique<ChaseTopkPrunedDecoder>();
      }
      return nullptr;
    });
  });
