...
template <int NBITS>
void decode_global_pair_qfloat(const DecodeRequest& request, DecodeResult& result) {
  ...
  if (!request.quiet) {
    std::cout << "[INFO] (" << request.label
              << ") Chase-global-pair decoder running in qfloat::qfloat<"
              << NBITS << ">\n";
  }
  ...
  auto decoded = ofec_decode_llr_global_pair(quantized, request.params,
                                             &result.tile_stats,
...
void decode_global_pair_quantized(const DecodeRequest& request,
                                  DecodeResult& result) {
  switch (request.quant_bits) {
    case 2:  decode_global_pair_qfloat<2 >(request, result); break;
    case 3:  decode_global_pair_qfloat<3 >(request, result); break;
    case 4:  decode_global_pair_qfloat<4 >(request, result); break;
    case 5:  decode_global_pair_qfloat<5 >(request, result); break;
    case 6:  decode_global_pair_qfloat<6 >(request, result); break;
    case 7:  decode_global_pair_qfloat<7 >(request, result); break;
    case 8:  decode_global_pair_qfloat<8 >(request, result); break;
    case 9:  decode_global_pair_qfloat<9 >(request, result); break;
    case 10: decode_global_pair_qfloat<10>(request, result); break;
    case 11: decode_global_pair_qfloat<11>(request, result); break;
    case 12: decode_global_pair_qfloat<12>(request, result); break;
    case 13: decode_global_pair_qfloat<13>(request, result); break;
    case 14: decode_global_pair_qfloat<14>(request, result); break;
    case 15: decode_global_pair_qfloat<15>(request, result); break;
    default:
      throw std::runtime_error(
          "[ERROR] Unsupported quant_bits for chase_global_pair decoder: " +
          std::to_string(request.quant_bits));
  }
}

class ChaseGlobalPairDecoder final : public IDecoder {
 public:
  DecodeResult decode(const DecodeRequest& request) override {
    ...
    switch (request.format) {
      case LlrFormat::Float:
        if (!request.quiet) {
          std::cout << "[INFO] (" << request.label
                    << ") Chase-global-pair decoder running in FLOAT\n";
        }
        if (!request.quiet) {
        ...
        result.pre_decoder_llr = request.channel_llr;
        result.post_decoder_llr =
            ofec_decode_llr_global_pair(result.pre_decoder_llr, request.params,
                                        &result.tile_stats,
                                        request.normalize_extrinsic,
                                        request.tx_llr_ref);
        break;
      ...
      case LlrFormat::Quantized:
        decode_global_pair_quantized(request, result);
        break;
    }
...
static void ensure_registered() {
  ...
  std::call_once(once, [] {
    register_decoder_factory([](const std::string& name)
      ...
      if (name == "chase_global_pair") {
        return std::make_unique<ChaseGlobalPairDecoder>();
      }
      return nullptr;
    });
  });
